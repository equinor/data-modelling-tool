...
class GetDocumentUseCase(uc.UseCase):
    ...
    def process_request(self, request_object: GetDocumentRequestObject):
        ...
        self.add_dtos(dtos, blueprint.attributes)
    ...
    def add_dtos(self, dtos, attribute):
        if "enumType" in attribute and len(attribute["enum_type"]) > 0:
            enum_blueprint = get_entity(attribute.enum_type)
            try:
                dtos.append(enum_blueprint)
            except AttributeError:
