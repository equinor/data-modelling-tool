...
def create_context_menu(
    ...
    if is_package:
        ...
        create_new_menu_items.append(
            get_dynamic_create_menu_item(
                data_source_id=data_source_id, name="Package", type=DMT.PACKAGE.value, node_id=node.node_id,
            )
        )
        ...
        for model in app_settings["models"]:
            ...
            create_new_menu_items.append(
                get_dynamic_create_menu_item(
                    data_source_id=data_source_id, name=model_blueprint.name, type=model, node_id=node.node_id,
                )
            )
    else:
        if node.is_array():
