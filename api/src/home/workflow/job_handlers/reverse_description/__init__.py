...
class JobHandler(JobHandlerInterface):
    ...
    def _insert_reference(self, document_id: str, reference: dict):
        req = requests.put(f"{config.DMSS_API}/api/v1/reference/{document_id}", json=reference, headers=self.headers)
        req.raise_for_status()
    ...
    def start(self) -> str:
        ...
        result = input
        result["description"] = input["description"][::-1]
        result["name"] = (
        ...
        self._insert_reference(
            document_id=self.job_entity["resultLinkTarget"],
            reference={"name": result["name"], "type": result["type"], "_id": add_response["uid"]},
        )
