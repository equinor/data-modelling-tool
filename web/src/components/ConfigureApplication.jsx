...
import { FaChevronDown, FaChevronUp, FaCog, FaPlus, FaMinus } from 'react-icons/fa'
import styled from "styled-components"
import Modal from "./modal/Modal"
import { SystemAPI } from "../api/Api3"
...
function Action({ value, index, onChange }) {
  ...
  if (collapsed) {
    return (
        <ActionWrapper style={{ cursor: "pointer" }} onClick={() => setCollapsed(false)}>
    ...
    return (
        <ActionWrapper>
          ...
          <TextInput label={"Name"} value={value.name} onChange={(e) => onChange({ ...value, name: e })}/>
          <TextInput label={"Description"} value={value.description}
                     onChange={(e) => onChange({ ...value, description: e })}/>
          <TextInput label={"Method"} value={value.method} onChange={(e) => onChange({ ...value, method: e })}/>
          <ActionTypesInput value={value.actionType} onChange={(e) => onChange({ ...value, actionType: e })}/>
  ...
  return (
      <>
        ...
        <ActionWrapper style={{ background: "white", margin: "0" }}>
          <PackagesSelector formData={value} onChange={value => onChange(value)}/>
        </ActionWrapper>
      </>)
}
  ...
  return (
      <>
        ...
        <Modal
            ...
            width={"50%"}
        >
          <TextInput label={"Name"} value={newSettings.name} onChange={(value) => onChange('name', value)}/>
          <TextInput label={"Description"} value={newSettings.description}
                     onChange={(value) => onChange('description', value)}/>
          <PackageInput label={"Entities"} value={newSettings.entities}
          ...
          {newSettings.actions && newSettings.actions.map((action, index) => {
            return <Action value={action} onChange={(value) => actionsOnChange(index, value)} index={index + 1}/>
          })}
