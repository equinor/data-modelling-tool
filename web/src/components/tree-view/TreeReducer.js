...
const expandNodesWithMatchingDescendants = (data, nodes, filter, matcher = defaultMatcher) => {
    return nodes.map(node => {
        let isOpen = false;
        if (node.children && node.children.length) {
            let childrenWithMatches = node.children.filter(child => nodeMatchesOrHasMatchingDescendants(data, data[child], filter, matcher));
...
const hideNodesWithNoMatchingDescendants = (data, filter, matcher = defaultMatcher) => {
...
export default (state, action) => {
    switch (action.type) {
        case FILTER_TREE:
            let filteredNodes = hideNodesWithNoMatchingDescendants(state, action.filter);
            let expandedNodes = expandNodesWithMatchingDescendants(state, filteredNodes, action.filter);
