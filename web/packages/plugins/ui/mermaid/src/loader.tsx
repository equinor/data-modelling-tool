...
const isPrimitive = (attribute: AttributeType) => ['string', 'number', 'integer', 'number', 'boolean'].includes(attribute.attributeType);
const primitiveAttributes = (blueprint: BlueprintType): AttributeType[] => blueprint.attributes.filter((attribute: AttributeType) => isPrimitive(attribute));
const isNonPrimitive = (attribute: AttributeType) => !isPrimitive(attribute);
const nonPrimitiveAttributes = (blueprint: BlueprintType): AttributeType[] => blueprint.attributes.filter((attribute: AttributeType) => isNonPrimitive(attribute));


const search = async (token: string, query: any) => {
    const dmssAPI = new DmssAPI(token)

    let result = await dmssAPI
        .searchDocuments({
...
export const loader = async (token: string, explorer: any, document: any): Promise<Node> => {
    const node = new Node(
        document
    )
    await Promise.all(nonPrimitiveAttributes(document)
        .map(async (attribute: AttributeType) => {
            const child: BlueprintType = await explorer.getBlueprint(attribute["attributeType"]);
            ...
            if (child["abstract"]) {
                ...
                await Promise.all(concreteDefinitions.map(async (concreteDefinition: BlueprintType) => {
