...
const classElement = (node: Node) => {
    const primitiveAttributeElements = node.primitiveAttributes()
        .map((attribute: AttributeType) => `${attribute.attributeType} ${attribute.name}`)
    return `
...
const PluginComponent = (props: DmtUIPlugin) => {
    ...
    useEffect(() => {
        loader(token, explorer, document)
            .then(async (tree: Node) => {
