  ...
  const { document } = props
    ...
    const getDocumentAttribute = (attributeName: string) => {
      ...
      if (document[attributeName]) {
          ...
          throw new Error(`Tried to access an attribute that does not exist on document` )
      }
    }
  ...
  return (
    <div>
      ...
      {getDocumentAttribute("attributes").map((attribute: any) => {
        ...
        <div style={{marginTop: "25px"}}>Does the blueprint have any storage recipes? {getDocumentAttribute("storageRecipes").length > 0 ? "Yes" : "No"}</div> {}
    </div>
  )
}
